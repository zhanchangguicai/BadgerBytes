            ...
            if(snapshot.val()==null){
                ...
                for(key in snapshot.val()){
                    if(snapshot.child(key).val().password==password_V){
                        if(snapshot.child(key).val().type=="admin" || snapshot.child(key).val().type=="staff"){
                            valid = true;
                            key_V = key;
                            type = snapshot.child(key).val().type;
                        }else{
                            type = "customer";
                ...
                if(valid){
                    if(type=="admin"){
                        ...
                        window.open("admin_control.html?key="+key_V+"&menuNum="+itemNum_V,"_self"); 
                    } else{
                        alert("Welcome, "+username_V+"!");
                        window.open("staff_control.html?key="+key_V+"&menuNum="+itemNum_V,"_self"); 
                    }
                }
            }
        });
